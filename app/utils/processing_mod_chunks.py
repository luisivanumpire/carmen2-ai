...
USE_FAISS = True     # Cambiar a False para usar similitud del coseno
...
def dividir_en_chunks(texto, tamano=300):
    palabras = texto.split()
    chunks = [" ".join(palabras[i:i + tamano]) for i in range(0, len(palabras), tamano)]
    return [c for c in chunks if len(c.split()) >= 20]

def generar_embeddings(lista_chunks):
    return modelo_embeddings.encode(lista_chunks, batch_size=32, show_progress_bar=False)
